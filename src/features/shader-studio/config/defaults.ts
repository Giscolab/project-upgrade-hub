...
export const PARAM_RANGE = {
  ...
  twist: { min: -5, max: 5, step: 0.01 },
  pulse: { min: 0, max: 10, step: 0.01 },
  morphFactor: { min: 0, max: 1, step: 0.01 },
  metalness: { min: 0, max: 1, step: 0.01 },
  rimPower: { min: 1, max: 12, step: 0.1 },
  fresnelStrength: { min: 1, max: 14, step: 0.1 },
...
export function formatStatus(params: ShaderParams) {
  return `Geometry: ${params.geometry} | Noise: ${params.noise} | Amp: ${params.amplitude.toFixed(2)} | Twist: ${params.material.twist.toFixed(2)}`;
}
